  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 relative overflow-hidden pb-20">
      ...
      <div className="container relative z-10 mx-auto px-4 lg:px-8 mt-4 flex w-full flex-col gap-3">
        ...
        {/* Compact Hero Area */}
        <div className="flex flex-col items-center text-center justify-center gap-3 mb-2 mt-4" style={{ animation: "fadeIn 0.5s ease-out both" }}>
        ...
        <div className="sticky top-4 z-50 flex flex-col items-center w-full">
          ...
          {/* Quick Toggle for Ranked */}
          {showFilters && (
            <div className="mt-4 flex animate-slide-down items-center gap-4 rounded-full border border-white/20 bg-white/70 backdrop-blur-2xl px-6 py-3 shadow-xl">
        ...
        <div className="mt-8 pb-20">
          {loading ? (
              ...
              {institutes.map((institute, index) => {
                ...
                return (
                  <div 
                    ...
                    style={{ animation: `fadeIn 0.6s cubic-bezier(0.23, 1, 0.32, 1) ${index * 0.04}s both` }}
                    onClick={() => router.push(`/institutes/${institute.institute_id}`)}
                  >
                    {/* Compact Premium Cover */}
                    <div className="relative h-32 w-full shrink-0 overflow-hidden bg-gradient-to-tr from-[var(--sp-purple)] via-[#8665aa] to-[var(--sp-blue)] bg-[length:200%_200%] animate-gradient-xy">
                    ...
                    <div className="relative -mt-6 mb-1 px-4 z-20">
                      <div className="h-12 w-12 overflow-hidden rounded-xl bg-white/80 backdrop-blur-xl p-1 shadow-[0_8px_20px_rgba(0,0,0,0.08)] ring-1 ring-white/50 transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-3">
                    ...
                    <div className="flex flex-1 flex-col px-4 pb-4 z-10 relative bg-white">
                      <h3 className="line-clamp-2 break-words text-[0.9rem] font-black tracking-tight leading-snug text-[var(--sp-ink)] group-hover:text-[var(--sp-purple)] transition-colors mb-2">
            ...
            <div className="flex flex-col items-center justify-center p-20 rounded-[4rem] border border-dashed border-slate-200 bg-white shadow-xl max-w-4xl mx-auto">
              <div className="h-20 w-20 mb-8 rounded-[2rem] bg-slate-50 flex items-center justify-center text-[var(--sp-purple)] shadow-inner">
                <Building2 className="h-10 w-10 opacity-20" />
              </div>
              <h3 className="text-2xl font-black text-[var(--sp-ink)]">Directory Match Error</h3>
              <p className="mt-2 text-sm font-medium text-[var(--sp-muted)] text-center max-w-sm">
                No institutions found for the current filter set. Try adjusting your geography or accreditation status.
              </p>
              <Button 
                variant="outline" 
                className="mt-8 rounded-full px-8 h-12 font-black uppercase tracking-widest text-[10px] border-2" 
                onClick={() => { setSearch(""); setCountryCode(""); setInstituteType(""); setAccreditationStatus(""); setRankedOnly(true); setCurrentPage(1); }}
              >
                Reset Exploration
              </Button>
            </div>
            ...
            <div className="mt-16 flex justify-center">
              <Pagination>
                <PaginationContent className="bg-white/80 backdrop-blur-md rounded-full border border-slate-200 p-1 shadow-lg ring-1 ring-black/5">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).filter(p => Math.abs(currentPage - p) < 2 || p === 1 || p === totalPages).map((p, idx, arr) => {
                    const isGap = idx > 0 && p - arr[idx - 1] > 1;
                    return (
                      <div key={p} className="flex items-center">
                        {isGap && <span className="px-2 text-slate-300 font-black">···</span>}
                        <PaginationItem>
                          <PaginationLink
                            onClick={() => setCurrentPage(p)}
                            isActive={currentPage === p}
                            className={cn(
                              "cursor-pointer rounded-full w-10 h-10 text-[10px] font-black transition-all border-0",
                              currentPage === p 
                                ? 'bg-[var(--sp-purple)] text-white shadow-[0_5px_15px_rgba(134,101,170,0.4)]' 
