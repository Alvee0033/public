...
const ScheduleCalendar = ({ schedules, onDateSelect, selectedDate, onEditSchedule }) => {
  ...
  const hasScheduleForDay = (day) => {
    ...
    // Check if this schedule has any work hours for any day of the week
  ...
  const getScheduleDetails = (day) => {
    ...
    // Find the first day of the week that has work hours
    const DAYS_ARRAY = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
    const firstWorkDay = DAYS_ARRAY.find(dayName => schedule[`${dayName}_start_time`] != null);
    
    if (!firstWorkDay) return null;

    const startTimeKey = `${firstWorkDay}_start_time`;
    const endTimeKey = `${firstWorkDay}_end_time`;
    const locationKey = `${firstWorkDay}_onsite_or_remote`;
    ...
    return {
      ...
      dayName: firstWorkDay.charAt(0).toUpperCase() + firstWorkDay.slice(1),
      schedule,
    };
  };
                  ...
                  >
                    <div className="bg-gray-900 text-white text-xs rounded-lg p-3 w-56 shadow-lg">
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <span>{scheduleDetails.isOnsite ? 'Onsite' : 'Remote'}</span>
                      </div>
                    </div>
                  </div>
                )}
