...
const RecentPosts = () => {
  ...
  return (
    <div className="p-5 md:p-30px lg:p-5 2xl:p-30px mb-30px border border-borderColor">
        ...
        {posts?.map(({ date, image, id, title }, idx) => (
          <li className="flex items-center" key={idx}>
            ...
            <div className="w-3/5">
              <Link
                href={`blogs/${id}`}
                className="w-full text-sm text-contentColor font-medium leading-7  hover:text-primaryColor"
              >
                {date}
              </Link>
