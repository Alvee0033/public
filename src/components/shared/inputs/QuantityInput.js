...
const QuantityInput = ({ quantity, setQuantity, type, product }) => {
  ...
  return (
    <div
      className={`max-w-150px h-55px leading-55px border-2 border-borderColor2   relative overflow-hidden ${
    ...
    >
      ...
      <div>
        <button
          className={`absolute left-[10px] top-1/2 -translate-y-1/2 text-blackColor  p-x-10px leading-1.8 w-5 inline-block ${
        ...
        <button
          className=" absolute top-1/2 -translate-y-1/2 right-[10px] text-blackColor  p-x-10px leading-1.8 w-5 inline-block"
          onClick={() => {
