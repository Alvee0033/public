...
const ProductModal = ({ product }) => {
  return (
    <div className="modal hidden fixed top-0 left-0 w-full h-full z-xxl transition-all duration-500 bg-lightBlack opacity-0 overflow-y-auto pb-10">
      ...
      <div className="modal-content transition-all duration-500 -translate-y-20 bg-whiteColor  p-15px max-w-250 mx-15px md:mx-auto mb-50px mt-110px md:my-150px relative z-small rounded-lg">
        <ProductDetails isModal={true} product={product} />
        {/* close button */}
        <div className="absolute top-4 right-3 z-xxl">
          <button className="modal-close group  rounded focus:shadow-select opacity-50 hover:opacity-75 p-1 inline-block">
            <svg
